...
closeArea()
{
	...
	if (level.zoneLastTime < 60)
		thread zoneMessage("^1CLOSING THE PLAY AREA IN " + level.zoneLastTime + " SEC");
	else
		thread zoneMessage("^1CLOSING THE PLAY AREA IN " + int(level.zoneLastTime / 60) + " MIN");
...
restrict()
{
	...
	while (seconds)
	{
		...
		if (seconds == 240)
			thread zoneMessage("^3RESTRICTING THE PLAY AREA IN 4 MIN");
		if (seconds == 120)
			thread zoneMessage("^3RESTRICTING THE PLAY AREA IN 2 MIN");
		else if (seconds < 60)
			thread zoneMessage("^3RESTRICTING THE PLAY AREA IN " + seconds + " SEC");
